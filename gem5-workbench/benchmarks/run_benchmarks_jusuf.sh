...
#
#wait
PYTHONPATH=bine-configs:${HOME}/.local/lib/python3.11/site-packages:$PYTHONPATH srun build/ALL/gem5.opt --no-output-files configs/multi-isa-nanogemm.py --isa aarch64 --mr {1..8} --nr {1..30} --simd_lat 4 6 10 --simd_count 1 2 4 --simd_width 128 256 512 1024 --decode_width 8 --commit_width 8 --fetch_buf_size 64 --assoc 8 --l1_size 64 --ld_count 2 --st_count 2 --simd_phreg_count 256 --rob_size 256 --iq_size 120 --split_bytes 5000000000 --base_out_dir=${SCRATCH}/nassyr1/nanogemm-$(date +'%F')-job${SLURM_JOBID} --quiet --sim_max_cores 256 --stat_filename stats_
