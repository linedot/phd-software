...
class O3_ARM_Neoverse_N1_ICache_128(Cache):
    tag_latency = 1
    data_latency = 1
    response_latency = 1
    mshrs = 8
...
class O3_ARM_Neoverse_N1_DCache_128(Cache):
    tag_latency = 3
    data_latency = 3
    response_latency = 1
    tgts_per_mshr = 16
...
class O3_ARM_Neoverse_N1_L2_128(Cache):
    tag_latency = 5
    data_latency = 5
    response_latency = 2 
    mshrs = 46  # (1)
...
class O3_ARM_Neoverse_N1_L3_128(L3Cache):
    tag_latency = 48 
    data_latency = 48
    response_latency = 16 
    assoc = 16 # (1)
...
class O3_ARM_Neoverse_N1_FP_128(FUDesc):
    ...
    opList = [ OpDesc(opClass='SimdAdd', opLat=2), # ASIMD arithmetic basis (add & sub)
               ...
               OpDesc(opClass='SimdReduceAdd', opLat=2), # SVE reduction, arithmetic, S form (saddv) 
               OpDesc(opClass='SimdReduceAlu', opLat=3), # SVE reduction, logical (andv)
               OpDesc(opClass='SimdReduceCmp', opLat=3), # SVE reduction, arithmetic, S form (smaxv)
               OpDesc(opClass='SimdFloatReduceAdd', opLat=2, pipelined=False), # SVE floating point associative add (fadda) // Same class for faddv, bad Gem5 implementation
               OpDesc(opClass='SimdFloatReduceCmp', opLat=3), # SVE floating point reduction f64 (fmaxv)
...
class O3_ARM_Neoverse_N1_Simple_Int_128(FUDesc):
...
class O3_ARM_Neoverse_N1_Complex_Int_128(FUDesc):
...
class O3_ARM_Neoverse_N1_LoadStore_128(FUDesc):
...
class O3_ARM_Neoverse_N1_PredAlu_128(FUDesc):
    opList = [ OpDesc(opClass='SimdPredAlu')  ]
    count = 1

class O3_ARM_Neoverse_N1_FUP_128(FUPool):
    FUList = [O3_ARM_Neoverse_N1_Simple_Int_128(),
              O3_ARM_Neoverse_N1_Complex_Int_128(),
              O3_ARM_Neoverse_N1_LoadStore_128(),
              O3_ARM_Neoverse_N1_PredAlu_128(),
              O3_ARM_Neoverse_N1_FP_128()]

# Bi-Mode Branch Predictor
class O3_ARM_Neoverse_N1_BP_128(BiModeBP):
...
class O3_ARM_Neoverse_N1_128(DerivO3CPU):
    ...
    fuPool = O3_ARM_Neoverse_N1_FUP_128()

    switched_out = False
    branchPred = O3_ARM_Neoverse_N1_BP_128()
